...
coding.AddMode.
	...
	FNC1Alpha      alphanumeric          any ASCII-compatible encoding
	ShiftJISKanji  kanji                 Shift JIS, Shift JISx0213
	ECI            eci                   N/A
	StructAppend   structured append     N/A
	FNC1First      fnc1 in 1st position  N/A
	FNC1Second     fnc1 in 2nd position  N/A
...
FNC1Alpha is like Alphanumeric but replaces "\x1d" with "%", for use
with strings following an FNC1First or FNC1Second Mode Segment.
FNC1String uses it in place of Alphanumeric automatically.

ECI, StructAppend, FNC1First and FNC1Second encode a raw segment.
They are not usable with Charset.  ECI Mode Segments are created by
SetECI, ShouldSetECI, MustSetECI, Text and FNC1Text.  FNC1First and
FNC1Second Mode Segments are created by SetFNC1 and FNC1Text.
StructAppend Mode Segments are generated by SplitMulti.
...
const (
	...
	FNC1Alpha     = coding.FNC1Alpha
	ECI           = coding.ECI
	StructAppend  = coding.StructAppend
	FNC1First     = coding.FNC1First
	FNC1Second    = coding.FNC1Second
...
type Splitter interface {
	// Split splits data into segments for the given QR version size
	// class and returns the Result and its encoded length in bits.
	// Split may be called more than once.  The Result may use the
	// same underlying data structure.
	Split(class int) (Result, int)

	// SplitTo returns the Result of up to lim bits and its length
	// for the beginning of the data, and a Splitter for the rest.
	// If no data fits in lim bits, the Result must be nil.  If all
	// data fits, the returned Splitter must be nil.  After SplitTo
	// the original Splitter is no longer usable.
...
// bare UTF-8 text.  Functions Text and FNC1Text construct Data for
...
func Split(data Data, level coding.Level, format Format) ([]coding.Segment, coding.Version, error) {
	...
	if level < coding.L || level > coding.H ||
		level == coding.H && format == Micro {
...
func (l splitList) SplitTo(lim, class int) (Result, int, Splitter) {
	...
	for i := range l {
		...
		n += nn
		if l[i].Result == nil {
			// Done.  No data in l[i] fits.
			if i == 0 {
				return nil, 0, l // No data fits
			}
			return l[:i], n, l[i:]
		}
		if l[i].Splitter != nil {
...
specified size, adding the initial part after the previous span.
After that it reverses the linked list.
*/
type (
	...
	segdata struct {
		...
		bits uint32   // encoded size of this and subsequent segments
	}
	...
	span struct {
		...
		best *segment   // segment with shortest encoding
	}
...
// FNC1String describes a string to encode after a segment of type
// FNC1First or FNC1Second.  It implements Data.
//
// String.Text must be in an ASCII-compatible encoding.  The FNC1 character
// is represented by the ASCII/UTF-8 Group Separator character "\x1d",
// and is encoded as is in byte mode or as "%" in alphanumeric mode.
// Percent sign "%" is encoded as "%%".
type FNC1String struct{ String }

func (s FNC1String) Splitter() (Splitter, int, error) {
	if s.String.Text == "" {
		return Null{}, 0, nil
	}
	s.String.Text = strings.ReplaceAll(s.String.Text, "%", "%%")

	// Get the Classifier function for the modes.
	if s.Charset == nil {
		s.Charset = UTF8
	}
	s.String.Charset = fnc1Charset{s.String.Charset}
	return s.String.Splitter()
}

// isRuneMode reports whether the mode's EncodedLength is rune-based.
func isRuneMode(mode coding.Mode) bool {
	if mode < ECI {
		return 1<<mode&(1<<Kanji|1<<Latin1) != 0
	} else {
		return 16 < mode.Length(0, 1, coding.Class0)
	}
}
...
func (v *span) add(next *span, class int, mask byte) *segment {
	...
	for j := range v.seg {
		...
		// - if next is nil, does not contain ns, or ns is over 4
...
type Charset interface {
	...
	// the given length in the lowest QR code version size class.
	MinLength(int) int
...
//	n  0x01  numeric mode
//
// chartbl is used by Classifier functions asciiCompat.classify,
// acceptsByte.classify, classifyByte, classifyRune and classifyLatin1
// to determine in which modes the character is encodable.
//
...
//
// The L bit sets byteMode for Latin1.  It is set on 0xc2 and 0xc3
...
// fnc1Charset is a wrapper around Charset for FNC1String.
type fnc1Charset struct{ Charset }

// fnc1Classifier is a wrapper aroung Classifier for FNC1String.
type fnc1Classifier struct {
	cf  Classifier // original Classifier
	bit byte       // alphanumeric mode bit
}

// classify adds alphanumeric mode to the modes returned by the
// original Classifier for the ASCII Group Separator character.
func (c fnc1Classifier) classify(s string) (byte, int) {
	m, sz := c.cf(s)
	if s[0] == 0x1d {
		m |= c.bit
	}
...
// Classifier replaces the Alphanumeric mode with FNC1Alpha and wraps
// the Classifier function returned by c.Classifier in fnc1Classifier.
func (c fnc1Charset) Classifier() (Classifier, ModeList, byte) {
	cf, m, mh := c.Charset.Classifier()
	for i := range m {
		if m[i] == Alphanumeric {
			m[i] = FNC1Alpha
			cf = fnc1Classifier{cf, 1 << i}.classify
			break
		}
	}
	return cf, m, mh
...
func NewCharset(m ModeList, digitLen int) Charset {
	c := &charset{modes: m, cut: utf8.DecodeRuneInString}
...
// If eci is negative, only the String is returned.
...
// SetFNC1 returns a Segment setting FNC1 mode.  If fnc1 is negative,
// the Segment's Mode is FNC1First, otherwise the Mode is FNC1Second
// and the codeword is set to fnc1.
func SetFNC1(fnc1 int) Data {
	var s Segment
	if fnc1 < 0 {
		s.Mode = FNC1First
	} else {
		s = Segment{string([]byte{byte(fnc1)}), FNC1Second}
	}
	return s
}

// FNC1Text returns a List containing an ECI Mode Segment, an FNC1
// segment and an FNC1String.  If eci is negative, the ECI Mode
// Segment is omitted.  If fnc1 is negative, the FNC1 segment's Mode
// is FNC1First, otherwise it's FNC1Second.
//
// text must be in an ASCII-compatible encoding, with FNC1 character
// represented by the Group Separator "\x1d" and percent sign by
// itself.
func FNC1Text(text string, c Charset, eci int, fnc1 int) Data {
	var (
		f = SetFNC1(fnc1)
		t = FNC1String{String{Text: text, Charset: c}}
		d Data
	)
	if eci >= 0 {
		d = List{ShouldSetECI(eci), f, t}
	} else {
		d = List{f, t}
	}
	return d
